...
class AzaleaAgentV9(AgentBase):
    """
    Agent V9 - Policy-only enhancements (no value head usage).

    Improvements over baseline:
    1. Lookup table swap (replaces broken neural swap)
    2. Temperature scaling with adaptive annealing
    3. 1-ply tactical check for immediate wins
    """
    ...
    def __init__(self, colour: Colour):
        ...
        # Higher temp = more exploration, lower = more exploitation
        self.initial_temperature = 0.5  # Start with some exploration
        self.min_temperature = 0.1      # Minimum temperature
        self.temperature_decay = 0.95   # Decay per move
    ...
    def _get_temperature(self, turn: int, total_cells: int = 121) -> float:
        """
        Calculate temperature based on game phase.

        Early game: Higher temperature for exploration
        Mid game: Moderate temperature
        Late game: Low temperature for precise play
        """
        # Estimate game progress (0.0 = start, 1.0 = end)
        moves_played = total_cells - (total_cells - turn + 1)
        progress = moves_played / total_cells
    ...
    def _select_move_neural(
        ...
        """
        Select move using pure policy head with temperature scaling.

        Temperature scaling:
        - temp=0: Pure argmax (greedy)
        - temp>0: softmax(logits/temp) sampling (exploration)
        """
        ...
        if temperature <= 0.05:
            # Near-zero temperature: use argmax
    ...
    def make_move(self, turn: int, board: Board, opp_move: Optional[Move]) -> Move:
        ...
        4. Neural policy with temperature scaling
        """
        ...
        # 4. Neural policy with temperature scaling
        best_move = self._select_move_neural(board, legal_moves, turn)

        return Move(best_move[0], best_move[1])


# For tournament compatibility
AzaleaAgentV9Class = AzaleaAgentV9
